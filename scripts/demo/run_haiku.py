...
Haiku is the project's joint encoder of H&E-derived features and clinical text —
not the LLM that powers HistoGen Advisor chat.

Combines per-patient ``clinical.json``, PHOENIX signature scores, and optional
GigaTIME outputs into ``demo/haiku/patients_embedding.json`` for similarity search
and the cluster graph in the UI.
"""
