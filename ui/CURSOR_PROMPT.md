...
| **Haiku** | Joint H&E + clinical-text encoder — patient embedding similarity, cluster card |
| **HistoGen Advisor** | Clinical LLM chat (demo: mock replies; production: separate LLM — not the Haiku encoder) |
...
1. **Left — Agent Chat**: HistoGen Advisor conversational interface (LLM; demo mock)
2. **Center — H&E / Spatial Viewer**: Modality toggle (H&E / RNA / Protein) + gene/protein dropdown + drag-drop slide loader
3. **Right — Patient Cluster & Clinical**: Haiku encoder cluster assignment, nearest similar patients, clinical tags
