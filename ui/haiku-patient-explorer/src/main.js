  ...
  app.innerHTML = `
    <header>
      <div>
        ...
        <p>Interactive TCGA lung TME explorer — click patients, search, explore signatures</p>
      </div>
...
function renderSurvival() {
  ...
  document.getElementById("survival-panel").innerHTML = `
    <div><strong>${p.case_id}</strong> — demo survival placeholder</div>
    <div>Status: <strong>${p.os_status}</strong> · Driver: <strong>${p.driver}</strong></div>
    <div class="survival-bar"><span style="width:${pct}%"></span></div>
    <div class="hint">Wire to TCGA clinical metadata for real KM curves.</div>
  `;
}
