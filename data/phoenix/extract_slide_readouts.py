#!/usr/bin/env python3
"""Extract PHOENIX spatial readouts for a TCGA diagnostic slide (demo-only path).

The bundled demo uses precomputed TCGA readouts in
``demo/phoenix/tcga-atlas-nest-multi-cell-20x-discrete.h5ad`` (fetch via
``fetch_demo_atlas.py``). For your own H&E slides use ``inference.py`` instead.

This script subsets that demo AnnData (``obs.slide`` holds TCGA case IDs such
...
Usage:
    python fetch_demo_atlas.py   # demo AnnData only (~23 GB)
...
def main() -> int:
    ...
    if not args.atlas.exists():
        raise SystemExit(
            f"Demo AnnData not found: {args.atlas}\nRun: cd {DATA_DIR} && python fetch_demo_atlas.py"
        )
