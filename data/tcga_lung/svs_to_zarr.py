...
def convert_svs_to_zarr(
    ...
    *,
    ...
    if zarr_path.exists():
        raise FileExistsError(f"Refusing to overwrite existing store: {zarr_path}")

    t0 = time.time()
    with SlideReader(svs_path) as slide:
        level_idxs = levels if levels is not None else list(range(slide.level_count))
